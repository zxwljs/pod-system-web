...
import ClientDownloadCard from './components/ClientDownloadCard.jsx'
...
// 公开下载页地址：下发给客户时使用（无需登录即可下载安装包，破解「无客户端→无法登录」死循环）
const DOWNLOAD_PAGE_URL = 'https://ddddnet.cn/#/download'

function parseHash() {
  const raw = window.location.hash.slice(1).split('?')[0]
  const parts = raw.split('/').filter(Boolean)
  return { root: parts[0] || null, sub: parts[1] || null }
}

function App() {
  const initial = parseHash()
  const [activeTab, setActiveTab] = useState(initial.root && TABS.includes(initial.root) ? initial.root : 'intro')
  const [introSubTab, setIntroSubTab] = useState(initial.root === 'intro' ? (initial.sub || 'about') : 'about')
  const [toolsSub, setToolsSub] = useState(initial.root === 'tools' ? initial.sub : null)
  ...
  // 公共视图（无需登录）：'landing' = 落地页（#/ 或无 hash），'login' = 登录页，
  // 'download' = 公开下载页（#/download，客户没有客户端也必须能拿到安装包）；null = 应用（需登录）
  const initialPublic = () => {
    const p = parseHash()
    if (p.root === 'login') return 'login'
    if (p.root === 'download') return 'download'
    if (!p.root) return 'landing'
    return null
  }
  const [publicView, setPublicView] = useState(initialPublic)
  ...
  const updateTabFromHash = () => {
    ...
    if (root === 'login') { setPublicView('login'); return }
    if (root === 'download') { setPublicView('download'); return }
    if (!root) { setPublicView('landing'); return }
    setPublicView(null)
    if (root && TABS.includes(root)) {
      setActiveTab(root)
      if (root === 'tools') setToolsSub(sub)
      if (root === 'intro') setIntroSubTab(sub || 'about')
    }
  }
  ...
  const navigateTo = (tabId, subId = null) => {
  ...
  // 公共页（无需登录即可访问）：落地页 / 登录页 / 下载页
  // 登录成功后回到业务首页；落地页的 CTA 指向登录页，下载页解决「没客户端无法登录」的死循环
  if (publicView === 'landing') {
    return <LandingPage siteSettings={siteSettings} onLogin={() => navigateTo('login')} />
  }
  if (publicView === 'download') {
    return (
      <DownloadPage
  ...
  if (publicView === 'login') {
    return (
      <LoginPage
        siteSettings={siteSettings}
        onBack={() => navigateTo('')}
        onLoggedIn={() => { window.location.hash = '#/patterns' }}
      />
    )
  }

  // 设计器是全屏独立页面，但同样必须过登录墙——此前直接 return 导致 #/designer 免鉴权
  if (activeTab === 'designer') {
    return (
      <LoginGate siteSettings={siteSettings}>
        <TemplateDesigner />
      </LoginGate>
    )
  }
  ...
  return (
    ...
    <div className="flex flex-col flex-1 bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <ConnectionStatus onNavigateToDownload={() => { navigateTo('intro', 'download') }} />
              <AccountMenu />
            </div>
          </div>
        </div>
      ...
      <main className={`flex-1 w-full mx-auto ${(activeTab === 'templates' || activeTab === 'templates-v2') ? 'max-w-7xl px-0 py-6' : 'max-w-7xl px-4 sm:px-6 lg:px-8 py-6'}`}>
        {activeTab === 'intro' && (
              ...
              <div className="flex space-x-8">
                {[
                  ...
                  return (
                    <button
                      key={subTab.id}
                      onClick={() => navigateTo('intro', subTab.id)}
                      className={`flex items-center space-x-2 px-4 py-4 border-b-2 font-medium transition-colors ${
                        isActive
                          ? 'border-blue-500 text-blue-600'
                          : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              ...
              {introSubTab === 'download' && (
                <div className="space-y-6">
                  ...
                  <p className="text-gray-600">
                    ...
                    下载地址随后台「软件版本」模块自动同步，始终指向最新版。
                  </p>

                  {/* 下载卡片：与公开下载页 #/download 共用同一组件，避免两处信息漂移 */}
                  <ClientDownloadCard siteSettings={siteSettings} />
