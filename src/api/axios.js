...
// ── 认证 token（从 auth.js 内存镜像取，避免 localStorage 磁盘 IO）──
// 只在本地后端需要的请求上附 Authorization（plan 6.3）
let _getAuthToken = () => null;
export const injectAuthTokenGetter = (fn) => { _getAuthToken = fn; };

// 供需要裸 fetch（如 blob 下载）的场景手动注入 Authorization
export const getAuthToken = () => _getAuthToken();
...
export const apiRequest = async (path, options = {}) => {
  ...
  // plan 6.3：注入 Authorization（本地后端 licenseGuard 要求）
  const extraHeaders = {};
  ...
  if (!response.ok) {
    ...
    // plan 6.3：401/403 标记 authFailure，供 LoginGate 全局捕获
    if (response.status === 401 || response.status === 403) {
      error.authFailure = true;
      console.warn('[api] 鉴权失败', response.status, errorCode, path);
      // 触发全局 forceLogout（LoginGate 挂载时注册 window.__pod_forceLogout）
      if (typeof window !== 'undefined' && typeof window.__pod_forceLogout === 'function') {
        window.__pod_forceLogout();
      }
    }
    throw error;
  }
...
export const apiUpload = async (path, formData, options = {}) => {
  ...
  const response = await fetch(`${getBaseURL()}${path}`, {
    method,
    body: formData,
    headers: { ...extraHeaders, ...(restOptions.headers || {}) },
    ...restOptions,
  });
