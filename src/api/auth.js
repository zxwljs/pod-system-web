...
// ── 轩宇汇登录策略（公开端点，不含敏感信息）──
// 供登录页按当前策略展示文案：approvalRequired=true 时新账号需管理员人工审批。
// 任何失败都静默返回 null（登录页用中性文案兜底）—— 策略拉取绝不能影响登录。
export async function fetchSsoPolicy() {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 5000);
  try {
    const resp = await fetch(AUTH_BASE_URL + '/api/auth/sso-policy', { signal: ctrl.signal });
    if (!resp.ok) return null;
    const data = await resp.json().catch(() => null);
    if (!data || typeof data.approvalRequired !== 'boolean') return null;
    const n = Number(data.trialDays);
    return { approvalRequired: data.approvalRequired, trialDays: Number.isFinite(n) && n >= 1 ? n : 30 };
  } catch (e) {
    return null;
  } finally {
    clearTimeout(timer);
