    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">查看套图 / 侵权检测 / 店小秘导出 / 工具箱（改尺寸·压缩·格式转换·Excel 合并）等任务的实时状态</p>
        </div>
        <button
      ...
      {tasks.length > 0 ? (
        <div className="space-y-4">
          {tasks.map(task => {
            ...
            const isResize = task.type === 'resize-batch';
            const isCompress = task.type === 'compress-batch';
            const isConvert = task.type === 'convert-batch';
            const typeLabel = isDxm ? '店小秘导出'
              : task.type === 'copyright-check' ? '侵权检测'
              : isExcelMerge ? 'Excel 合并'
              : isResize ? '批量改尺寸'
              : isCompress ? '批量压缩'
              : isConvert ? '图片格式转换'
              : '批量套图';
            const typeBadgeColor = isDxm ? 'bg-purple-100 text-purple-700'
              : task.type === 'copyright-check' ? 'bg-amber-100 text-amber-700'
              : isExcelMerge ? 'bg-green-100 text-green-700'
              : isResize ? 'bg-blue-100 text-blue-700'
              : isCompress ? 'bg-teal-100 text-teal-700'
              : isConvert ? 'bg-orange-100 text-orange-700'
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-gray-500 mt-2">在"图案库"中点击"批量套图"/"侵权检测"，或在"工具箱"运行改尺寸/压缩/格式转换/Excel 合并，或在结果页导出"店小秘上架表格"后，会在这里显示进度</p>
        </div>
      )}
    </div>
  );
