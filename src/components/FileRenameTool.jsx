...
function FileRenameTool() {
  ...
  // 由 Python 启动器「选文件夹并打开」跳转而来：读取 path 参数回填
  // 优先级高于 localStorage 记忆；同时监听 hashchange 以便重复选择时刷新
  useEffect(() => {
    const applyHashPath = () => {
      try {
        ...
        if (p) setFolderPath(decodeURIComponent(p))
      } catch {}
    }
    applyHashPath()
    window.addEventListener('hashchange', applyHashPath)
    return () => window.removeEventListener('hashchange', applyHashPath)
  }, [])
  ...
  const handleNativePick = () => {
    // 尝试唤起桌面客户端的 Windows 原生选择窗口
    window.location.href = 'pod://select-folder'
    // 浏览器无法知道外部协议是否成功处理，给一个中性提示即可
    // （成功唤起时页面不会离开；协议未注册时浏览器通常也无反应）
    setError('')
    setPickHint('已尝试唤起桌面客户端。请在弹出的 Windows 窗口中选择文件夹；若未弹出，请确保已运行 POD套图系统.exe。')
    setTimeout(() => setPickHint(''), 4000)
  }
            ...
            <div>
              ...
              <div className="flex space-x-2">
                ...
                <button
                  ...
                  title="弹出 Windows 原生文件夹选择窗（需已运行桌面客户端）"
                >
                ...
                <button
                  ...
                  title="在网页内浏览目录（无需桌面客户端）"
                >
                  <HardDrive className="w-5 h-5" />
                </button>
              </div>
              <p className="text-xs text-gray-400 mt-1">
                点击文件夹图标用桌面客户端原生窗口选路径；或点蓝色按钮在网页内浏览
              </p>
              {pickHint && (
