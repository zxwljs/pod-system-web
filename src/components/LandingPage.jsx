// ─── 公共落地页（未登录可见）───
// 目的：① 让陌生访客先知道这是什么产品 ② 展示套餐与客服 ③ 提供登录入口 ④ 提供客户端下载入口
// 注意：本页不出现任何后端/授权服务地址，避免为攻击者提供侦察信息。

import React, { useState, useEffect } from 'react';
import { Download, LogIn, ExternalLink, UserPlus } from 'lucide-react';
...
// 公开下载页（无需登录）：客户拿到账号但还没装客户端时，必须先能自助下载
const DOWNLOAD_PAGE_HASH = '#/download';
...
const ONBOARDING = [
  { title: '下载客户端', desc: '本页右上角「下载客户端」，或直接访问 ddddnet.cn/#/download' },
  { title: '双击运行', desc: '安装包无需安装，双击 .exe 即可在本地启动' },
  { title: '登录使用', desc: '用我们分配的账号登录，网页会自动连接本机客户端' },
];
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a
              href={DOWNLOAD_PAGE_HASH}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-gray-700 text-sm font-medium border border-gray-200 hover:bg-gray-50 transition-colors"
            >
      ...
      <section className="bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        <div className="max-w-6xl mx-auto px-6 py-20 text-center">
          ...
          <div className="mt-9 flex flex-col sm:flex-row gap-3 justify-center">
            <button
              onClick={onLogin}
              className="inline-flex items-center justify-center gap-2 px-7 py-3 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors"
            >
            ...
            <a
              href={DOWNLOAD_PAGE_HASH}
              className="inline-flex items-center justify-center gap-2 px-7 py-3 rounded-xl bg-white border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors"
            >
              <Download className="w-4 h-4" />
              下载客户端
            </a>
          ...
          <p className="mt-4 text-sm text-gray-500">
            桌面客户端运行在本机，图片处理不上传云端，处理更快也更安全
          </p>
          {/* 明确「先装客户端 → 再登录」的顺序：客户刚拿到账号时最容易卡在这一步 */}
          <div className="mt-6 inline-flex flex-wrap items-center justify-center gap-2 rounded-full bg-white/80 border border-gray-200 px-4 py-2 text-sm text-gray-600">
            <UserPlus className="w-4 h-4 text-blue-600" />
            <span>新客户还没有客户端？</span>
            <a href={DOWNLOAD_PAGE_HASH} className="inline-flex items-center gap-1 font-medium text-blue-600 hover:underline">
              先下载安装包
      ...
      <section className="border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 py-12">
          <h2 className="text-lg font-semibold text-gray-900 text-center">三步开始使用</h2>
      ...
      <footer className="border-t border-gray-100 py-8">
        <div className="max-w-6xl mx-auto px-6 text-center text-xs text-gray-400 space-y-2">
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            <a href={DOWNLOAD_PAGE_HASH} className="hover:text-gray-600 transition-colors">下载客户端</a>
            <button onClick={onLogin} className="hover:text-gray-600 transition-colors">登录</button>
            <a href="https://www.ddddnet.cn" className="hover:text-gray-600 transition-colors">了解产品</a>
            <a href="/terms.html" target="_blank" rel="noopener noreferrer" className="hover:text-gray-600 transition-colors">用户服务协议</a>
            <a href="/terms.html#privacy" target="_blank" rel="noopener noreferrer" className="hover:text-gray-600 transition-colors">隐私政策</a>
          </div>
