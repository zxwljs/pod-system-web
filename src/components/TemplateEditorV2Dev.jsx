...
import {
  ...
  Image as ImageIcon, Layers, Settings, Copy, GripVertical, FolderOpen
} from 'lucide-react';
import {
  DndContext,
  closestCenter,
...
function TemplateEditorV2() {
  ...
  // ── 遮挡工具状态 ──
  const [editMode, setEditMode] = useState('printArea'); // 'printArea' | 'occlusion'
  ...
  const handleAreaMouseDown = useCallback((e, area, type, isDetail = false) => {
    e.stopPropagation();
    // 遮挡模式：点击印花框 = 选中目标 + 在该位置添加多边形顶点
    if (editMode === 'occlusion' && !isDetail) {
  ...
  const finishOcclusionPolygon = () => {
    ...
    const targetArea = printAreas.find(a => a.templateId === selectedOcclusionTarget);
    ...
    // 将多边形添加到对应 printArea 的 occlusionMasks
    setSkus(prev => prev.map((sk, idx) => {
      if (idx !== activeSkuIndex) return sk;
      const areas = (sk.mainPrintAreas || []).map(a => {
        if (a.templateId !== selectedOcclusionTarget) return a;
        const masks = a.occlusionMasks || [];
        return { ...a, occlusionMasks: [...masks, [...occlusionPoints]] };
      });
      return { ...sk, mainPrintAreas: areas };
    }));
    setOcclusionPoints([]);
  };
  ...
  const removeOcclusionMask = (areaId, maskIndex) => {
    setSkus(prev => prev.map((sk, idx) => {
      if (idx !== activeSkuIndex) return sk;
      const areas = (sk.mainPrintAreas || []).map(a => {
        if (a.templateId !== areaId) return a;
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-1 px-3 py-1.5 rounded-lg border border-amber-300 bg-amber-50 text-amber-700 text-sm font-medium hover:bg-amber-100 transition-colors"
            >
            ...
            {templatePickerOpen && (
              <div className="absolute left-0 top-full mt-1 w-72 max-h-80 overflow-y-auto bg-white rounded-xl shadow-lg border border-gray-200 z-50 py-1">
                <div className="px-3 py-1.5 text-xs text-gray-400 border-b border-gray-100">
                  共 {allTemplates.length} 个模板 · 点击切换（当前页不保存直接跳转）
                </div>
                {allTemplates.length === 0 && (
                  <div className="px-3 py-3 text-sm text-gray-400">加载中…</div>
                )}
                {allTemplates.map(t => {
                  const isCurrent = template && t.id === template.id;
                  return (
                    <button
                      key={t.id}
                      onClick={() => {
                        ...
                        if (window.confirm(`切换到模板「${t.name}」？当前未保存的改动将丢失。`)) {
                          window.location.hash = `#/designer-dev?id=${t.id}`;
                          window.location.reload();
                        }
                      }}
                      className={`w-full text-left px-3 py-2 hover:bg-amber-50 transition-colors ${
                        isCurrent ? 'bg-amber-50 text-amber-700' : 'text-gray-700'
                      }`}
                    >
                      <div className="text-sm font-medium truncate">{t.name || '(未命名)'}</div>
                      <div className="text-xs text-gray-400">
                        {((t.colors || []).length) || 0} 个颜色
                        {t.occlusionMasks || (t.colors || []).some(c => (c.printAreas || []).some(pa => pa.occlusionMasks?.length)) ? ' · 含遮挡区' : ''}
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex-1 flex items-center justify-center p-4 overflow-auto">
                ...
                >
                  ...
                  {/* ── 遮挡多边形 SVG 渲染 ── */}
                  {editMode === 'occlusion' && designerMode === 'main' && (
                    <div className="absolute inset-0 pointer-events-none" style={{ width: currentCanvasSize.width * zoom, height: currentCanvasSize.height * zoom }}>
                      <svg width="100%" height="100%" style={{ overflow: 'visible' }}>
                        {/* 已保存的遮挡多边形 */}
                        {printAreas.flatMap(area =>
                          (area.occlusionMasks || []).map((pts, mi) => {
                            const points = pts.map(p => `${(p.x * zoom).toFixed(1)},${(p.y * zoom).toFixed(1)}`).join(' ');
                            const isSelected = selectedOcclusionTarget === area.templateId;
                            return (
                              <polygon key={`${area.templateId}-${mi}`} points={points}
                        ...
                        {occlusionPoints.length > 0 && (
                          <>
                            {occlusionPoints.length >= 2 && (
                              <polyline
                                points={occlusionPoints.map(p => `${(p.x * zoom).toFixed(1)},${(p.y * zoom).toFixed(1)}`).join(' ')}
                                fill="rgba(239,68,68,0.08)" stroke="#ef4444" strokeWidth={1.5} strokeDasharray="5 3"
                              />
                            )}
                            {occlusionPoints.map((p, i) => (
                              <circle key={i} cx={(p.x * zoom).toFixed(1)} cy={(p.y * zoom).toFixed(1)}
                                r={i === 0 ? 6 : 4}
                ...
                <div className="bg-white border-t border-red-100 px-4 py-3 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium text-gray-900">遮挡绘制模式</span>
                      {occlusionPoints.length > 0 && (
                      ...
                      {occlusionPoints.length >= 3 && selectedOcclusionTarget && (
                        occlusionOverlapsArea(occlusionPoints, printAreas.find(a => a.templateId === selectedOcclusionTarget))
                  ...
                  {selectedOcclusionTarget ? (
                    <div className="flex items-center gap-4 flex-wrap">
                      <span className="text-xs text-gray-500">
                        目标区域：{printAreas.find(a => a.templateId === selectedOcclusionTarget)?.name || '未知'}
                      </span>
                      <div className="flex items-center gap-2">
                        <label className="text-xs text-gray-500">羽化</label>
                        <input type="range" min="0" max="6" step="0.5" value={printAreas.find(a => a.templateId === selectedOcclusionTarget)?.occlusionFeather || 0}
                          onChange={e => setOcclusionFeatherForArea(selectedOcclusionTarget, parseFloat(e.target.value))}
                          className="w-24 h-1"
                        />
                        <span className="text-xs text-gray-600 w-8">{printAreas.find(a => a.templateId === selectedOcclusionTarget)?.occlusionFeather || 0}px</span>
                      </div>
                    </div>
                  ) : (
                    <p className="text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
                      ① 先点击一个印花框（蓝色）选中目标区域 → ② 再在画布上点击绘制遮挡多边形。<b>遮挡区必须覆盖到印花区内部才会产生挖空效果</b>（帽兜/口袋压住图案的位置）。
                    </p>
                  )}

                  {/* 遮挡列表 */}
                  {selectedOcclusionTarget && (() => {
                    const area = printAreas.find(a => a.templateId === selectedOcclusionTarget);
