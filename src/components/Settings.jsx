...
import { apiGetCopyrightSettings, apiSaveCopyrightSettings, apiGetTitlePrompts, apiSaveTitlePrompts, apiGetSaveConfig, apiSaveSaveConfig, apiValidateSaveConfig,
  ...
  apiGetColorAliases, apiCreateColorAlias, apiDeleteColorAlias,
  apiGetShops, apiCreateShop, apiUpdateShop, apiDeleteShop, apiRequest } from '../api/axios'
import { getDuplicateSettings, saveDuplicateSettings, clearDuplicateIndex } from '../api/duplicate'
import { Save, Key, BookOpen, AlertCircle, CheckCircle2, Info, FolderOutput, Folder, Sparkles, Settings2, Plus, Trash2, Edit3, Zap, Copy, RefreshCw, Store, X, Pencil } from 'lucide-react'
...
function Settings() {
  ...
  const [shopForm, setShopForm] = useState({ name: '', platform: '', remark: '', outputPath: '' })
  const [editingShopId, setEditingShopId] = useState(null)
  const [shopMessage, setShopMessage] = useState('')
  const [shopMessageType, setShopMessageType] = useState('info')
  ...
  const [dupSettings, setDupSettings] = useState({
    ...
    ignoreSameGroupDuplicates: false,
  ...
  const [resettingDup, setResettingDup] = useState(false)
  ...
  const [devMsgType, setDevMsgType] = useState('info')
  ...
  useEffect(() => {
    if (activeSecondTab === 'temu') {
      ...
      apiRequest('/templates-v2').then(arr => setMockupTemplates(Array.isArray(arr) ? arr : [])).catch(() => {})
    }
  }, [activeSecondTab])
    ...
    const applyHashTab = () => {
      try {
        ...
        if (tab === 'duplicate') {
          ...
          if (third) setActiveThirdTab(third)
        } else if (tab === 'general') {
          setActiveSecondTab('general')
          if (third && thirdTabs.general.some(t => t.id === third)) {
            setActiveThirdTab(third)
          }
        }
      } catch {}
    }
  ...
  const handleResetDupIndex = async () => {
    if (!window.confirm('确定要重置查重数据吗？这将清空所有已生成的重复分组索引，之后需要重新全库扫描。')) return
    setResettingDup(true)
    setDupMsg('')
    try {
      await clearDuplicateIndex()
      setDupMsg('查重数据已重置，请重新扫描')
      setDupMsgType('success')
      window.open('#/patterns?tab=duplicate', '_blank')
    } catch (e) {
      setDupMsg('重置失败: ' + e.message)
      setDupMsgType('error')
    } finally {
      setResettingDup(false)
    }
  }
  ...
  const handleSaveShop = async () => {
    if (!shopForm.name.trim()) {
      ...
      setShopMessage('请填写店铺名称')
      return
    }
    const trimmedOutput = (shopForm.outputPath || '').trim()
    if (trimmedOutput) {
      const validation = await apiValidateSaveConfig(trimmedOutput)
      if (!validation.valid) {
        setShopMessageType('error')
        setShopMessage('导出路径无效: ' + (validation.reason || '未知原因'))
        return
      }
    }
    setSavingShop(true)
    try {
      ...
      if (editingShopId) {
        await apiUpdateShop(editingShopId, payload)
      } else {
        await apiCreateShop(payload)
      }
      await loadShops()
      setShopForm({ name: '', platform: '', remark: '', outputPath: '' })
      setEditingShopId(null)
      setShopMessageType('success')
      setShopMessage(editingShopId ? '保存成功' : '创建成功')
      setTimeout(() => setShopMessage(''), 3000)
    } catch (error) {
      console.error('保存店铺失败:', error)
      setShopMessageType('error')
      setShopMessage('保存失败: ' + (error.message || '请重试'))
    } finally {
      setSavingShop(false)
    }
  }
  ...
  const handleEditShop = (shop) => {
    ...
    setShopForm({
      ...
      remark: shop.remark || '',
      outputPath: shop.outputPath || ''
    })
  }

  const handleDeleteShop = async (shopId) => {
    if (!confirm('确定删除该店铺吗？关联的文件夹将变为"共享"状态，不会被删除。')) return
    try {
      ...
      alert('删除店铺失败: ' + (error.message || '请重试'))
    }
  }

  const handleClearShopForm = () => {
    setEditingShopId(null)
    setShopForm({ name: '', platform: '', remark: '', outputPath: '' })
    setShopMessage('')
  }
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 rounded-xl border border-blue-200 p-4">
        <div className="flex items-start space-x-3">
          <Info className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
          <div className="text-sm text-blue-700">
            <p className="font-medium">店铺管理与导出路径</p>
            <p className="mt-1">在这里创建/编辑店铺，并为每个店铺单独设置「保存到本地」的导出路径。</p>
            <p className="mt-1">优先级规则：文件夹只属于一个店铺且该店铺配置了导出路径 → 用店铺路径；否则回退到上方的「默认输出路径」。</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-900">{editingShopId ? '编辑店铺' : '新建店铺'}</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">平台（选填）</label>
            <input
              type="text"
              value={shopForm.platform}
              onChange={(e) => setShopForm(prev => ({ ...prev, platform: e.target.value }))}
              placeholder="如 Temu、Shein、独立站"
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setShopForm(prev => ({ ...prev, remark: e.target.value }))}
              placeholder="备注信息"
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">导出路径（选填）</label>
            <input
              type="text"
              value={shopForm.outputPath}
              onChange={(e) => setShopForm(prev => ({ ...prev, outputPath: e.target.value }))}
              placeholder="如 D:\\output\\shop-a，不填则回退默认路径"
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-end space-x-4 mt-6">
          {shopMessage && (
            <span className={`text-sm ${shopMessageType === 'success' ? 'text-green-600' : shopMessageType === 'error' ? 'text-red-600' : 'text-gray-600'}`}>
              {shopMessage}
            </span>
          )}
          {editingShopId && (
            <button
              onClick={handleClearShopForm}
          ...
          >
            <Save className="w-4 h-4" />
            <span>{savingShop ? '保存中...' : (editingShopId ? '保存修改' : '创建店铺')}</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          <Store className="w-5 h-5 text-purple-600" />
          <h3 className="font-semibold text-gray-900">店铺列表</h3>
        </div>
        {shops.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-8">暂无店铺，在上方创建第一个店铺吧</p>
        ) : (
          <div className="space-y-3">
            {shops.map(shop => (
              <div key={shop.id} className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-medium ${getShopColor(shop.id)}`}>
                      <Store className="w-3 h-3 mr-1" />
                      {shop.name}
                    </span>
                    {shop.platform && <span className="text-xs text-gray-500">{shop.platform}</span>}
                    <span className="text-xs text-gray-400">{shop.folderCount || 0} 个文件夹</span>
                    {shop.outputPath ? (
                      ...
                      <span className="text-xs text-gray-400">未配置导出路径，使用默认路径</span>
                    )}
                  </div>
                  {shop.remark && <p className="text-xs text-gray-500 mt-2 truncate">{shop.remark}</p>}
                </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-5">
            ...
            <div className="py-2 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-gray-900">同组内相同图不提醒</div>
                  <div className="text-xs text-gray-500 mt-0.5">同一套图组内（如衣服前/后幅 0001-1、0001-2）的相同图视为故意重复，不报重；仍会抓跨文件夹及同文件夹不同组的重复。建议配合「检测范围=全部」使用。</div>
                </div>
                <Toggle
                  checked={!!dupSettings.ignoreSameGroupDuplicates}
                  onChange={v => updateDup('ignoreSameGroupDuplicates', v)}
                  disabled={!dupSettings.enabled}
                />
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-red-200 p-5">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="font-medium text-gray-900">重置查重数据</p>
              <p className="text-sm text-gray-600 mt-1">清空已生成的重复分组索引。当文件夹店铺归属调整、担心旧索引跨店误报或漏报时，可重置后重新全库扫描。</p>
              <button
                onClick={handleResetDupIndex}
                disabled={resettingDup}
                className="mt-3 inline-flex items-center space-x-1.5 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
                {resettingDup ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                <span>{resettingDup ? '重置中...' : '重置查重数据'}</span>
              </button>
            </div>
          </div>
        </div>
  ...
  const renderDevTab = () => {
    const run = async (apply) => {
      if (apply && !window.confirm('即将把跨目录引用的图片复制到各自模板目录并修正数据库配置（操作前数据库会自动备份）。确定执行修复吗？')) return
      setDevScanning(true)
      setDevMsg('')
      setDevReport(null)
      try {
        const data = await apiRequest(`/templates-v2/repair-crossdir?apply=${apply ? '1' : '0'}`, { method: 'POST' })
        setDevReport(data)
        if (data.summary.missing > 0) {
          setDevMsg(`检测到 ${data.summary.missing} 张图片源文件已缺失，无法自动修复（需在仍保留源图的源环境执行）。可修复 ${data.summary.fixable} 张。`)
          setDevMsgType('error')
        } else if (data.summary.fixable > 0) {
          setDevMsg(apply ? `已修复 ${data.summary.fixable} 张跨目录图片引用，回到模板管理即可正常导出。` : `预览：可修复 ${data.summary.fixable} 张。点击「检测并修复」执行。`)
          setDevMsgType('success')
        } else {
          setDevMsg('未发现跨目录图片引用问题，无需修复')
          setDevMsgType('success')
        }
      } catch (e) {
        setDevMsg('操作失败: ' + e.message)
        setDevMsgType('error')
      } finally {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl shadow-sm border border-indigo-200 p-5">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <p className="font-medium text-gray-900">遮挡设计器（开发版）</p>
                <span className="px-1.5 py-0.5 text-[10px] font-bold bg-indigo-600 text-white rounded">DEV</span>
              </div>
              <p className="text-sm text-gray-600 mt-1">
                在模板印花区上绘制遮挡多边形（帽兜、口袋等），套图时图案自动避开遮挡区域。开发测试中，不影响正式设计器。
              </p>
              <div className="mt-3">
                <button
                  onClick={() => { window.location.hash = '#/designer-dev' }}
                  className="inline-flex items-center space-x-1.5 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors"
                >
                  <span>进入遮挡设计器</span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-purple-200 p-5">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="font-medium text-gray-900">模板图片跨目录引用修复</p>
              <p className="text-sm text-gray-600 mt-1">
                早期复制模板时，细节图/主图的 imagePath 可能指向了其他模板目录。导出只打包自身目录，导致这些图缺失、导入方得到不完整的模板（空壳包）。
                此工具会扫描<strong>全部模板</strong>，把跨目录引用的图片复制回各自模板目录并修正配置（通用适配所有底板，不局限于某一块）。
              </p>
              <div className="mt-4 flex items-center space-x-3">
                <button
                  onClick={() => run(false)}
                  disabled={devScanning}
                  className="inline-flex items-center space-x-1.5 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {devScanning ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Info className="w-4 h-4" />}
                  <span>{devScanning ? '检测中...' : '检测（仅预览）'}</span>
                </button>
                <button
                  onClick={() => run(true)}
                  disabled={devScanning}
                  className="inline-flex items-center space-x-1.5 px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {devScanning ? <RefreshCw className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                  <span>{devScanning ? '修复中...' : '检测并修复'}</span>
                </button>
              </div>
              {devMsg && (
                <span className={`mt-3 inline-flex items-center space-x-1 text-sm ${devMsgType === 'success' ? 'text-green-600' : 'text-red-600'}`}>
                  {devMsgType === 'success' ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                  <span>{devMsg}</span>
                </span>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div className="flex items-center space-x-6 mb-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-600">{devReport.summary.fixable}</div>
                <div className="text-xs text-gray-500">可修复</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{devReport.summary.missing}</div>
                <div className="text-xs text-gray-500">源图缺失</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-600">{devReport.summary.skipped}</div>
                <div className="text-xs text-gray-500">已正常</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-amber-600">{devReport.summary.renamed}</div>
                <div className="text-xs text-gray-500">冲突改名</div>
              </div>
            </div>

            {devReport.fixed.length > 0 && (
              <div className="mt-2">
                <p className="text-sm font-medium text-gray-900 mb-2">修复项（{devReport.fixed.length}）</p>
                <div className="max-h-60 overflow-auto rounded-lg border border-gray-100 divide-y divide-gray-100">
                  {devReport.fixed.map((f, i) => (
                    <div key={i} className="px-3 py-2 text-xs">
                      <span className="text-gray-500">[{f.label}]</span> <span className="font-medium text-gray-800">{f.tpl}</span>
                      <div className="text-gray-400 mt-0.5 break-all">{f.from} <span className="text-gray-300">→</span> {f.to}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {devReport.missing.length > 0 && (
              <div className="mt-4">
                <p className="text-sm font-medium text-red-600 mb-2">源图缺失，无法自动修复（{devReport.missing.length}）—— 请在存有这些图的源环境执行修复，或在模板编辑中重新上传这些细节图</p>
                <div className="max-h-60 overflow-auto rounded-lg border border-red-100 divide-y divide-red-100 bg-red-50/40">
                  {devReport.missing.map((m, i) => (
                    <div key={i} className="px-3 py-2 text-xs">
                      <span className="text-gray-500">[{m.label}]</span> <span className="font-medium text-gray-800">{m.tpl}</span>
                      <div className="text-gray-400 mt-0.5 break-all">{m.from}</div>
                    </div>
                  ))}
                </div>
              </div>
