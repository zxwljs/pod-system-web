...
function PatternManager() {
  ...
  const [outlineThickness, setOutlineThickness] = useState(1) // 描边粗细 px，支持小数（0.2 ~ 6），默认 1px
  const [invertFallback, setInvertFallback] = useState(false) // 反色失败是否兜底描边
  const [invertMode, setInvertMode] = useState('color') // 智能反色反相模式: 'color'(彩色反相) | 'mono'(纯黑白反色, 按底色自动选黑/白)
    ...
    try {
      const data = await apiRequest('/preview-color', {
        ...
        data: {
          ...
          invertMode: effectiveSkuInvertMode(name),
  ...
  // 该 SKU 反色时的反相模式：显式设置 > 全局 invertMode
  const effectiveSkuInvertMode = (name) => {
    const e = skuRepairEntry(name)
    return e.invertMode === 'mono' ? 'mono' : invertMode
  }
  ...
  const fetchGlobalInvertPreview = async () => {
    ...
    try {
      const [before, after] = await Promise.all([
        ...
        apiRequest('/preview-color', { method: 'POST', data: { folderId: selectedFolder.id, templateId: selectedTemplateId, colorName: repColor.name, repairMode: 'invert', outlineStyle, invertFallback, conflictSensitivity, invertMode } })
      ])
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [repairMode, selectedTemplateId, selectedFolder && selectedFolder.id, outlineStyle, invertFallback, conflictSensitivity, invertMode])
              ...
              <div className="px-4 py-3 bg-white rounded-lg border border-gray-200">
                    ...
                    {repairSettingsCollapsed && (
                      <div className="mt-1.5 text-[10px] text-blue-600 font-medium">
                        {(() => {
                          ...
                          return `当前：智能反色 · ${invertMode === 'mono' ? '纯黑白反色' : '彩色反相'} · ${invertFallback ? '失败兜底描边' : '保留原样'} · 敏感度${sensLabel}`
                        })()}
                      </div>
                    )}
                  </div>
                ...
                {repairMode === 'invert' && (
                  <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-2.5 space-y-2.5">
                    {/* 0 反相模式：彩色反相 / 纯黑白反色（按底色自动选黑或白） */}
                    <div>
                      <div className="text-xs font-medium text-emerald-800 mb-1.5">反相模式</div>
                      <div className="flex gap-2">
                        {[
                      ...
                      {invertMode === 'mono' && (
                        <div className="mt-1 text-[10px] text-gray-500 leading-tight">系统按衣服底色亮度自动选黑或白，保证 logo 清晰</div>
                      )}
                    </div>
                    {/* ① 展示区：智能反色效果（真实预览前后对比） */}
                    <div>
                ...
                {(() => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center space-x-1 px-2 py-0.5 rounded-full text-[11px] font-medium text-white shadow-sm transition-transform hover:scale-105 ${isExact ? 'bg-red-500' : 'bg-orange-500'}`}
