...
import { getDeviceId, authLogin, setAuthToken, pollSsoTicket, authSsoLogin, fetchSsoPolicy } from '../api/auth.js';
...
export default function LoginPage({ onLoggedIn, siteSettings, onBack }) {
  ...
  // 轩宇汇登录策略（公开端点提供）：null = 未取到，用中性文案兜底
  const [ssoPolicy, setSsoPolicy] = useState(null);
  useEffect(() => {
    let alive = true;
    fetchSsoPolicy().then((p) => { if (alive) setSsoPolicy(p); });
    return () => { alive = false; };
  }, []);
  // 文案随策略切换，避免后台切到「需人工审批」后仍宣称自动开通
  const ssoTip = !ssoPolicy
    ? '轩宇汇账号首次登录可开通免费试用或等待管理员开通'
    : ssoPolicy.approvalRequired
      ? '轩宇汇账号首次登录需等待管理员开通，请联系客服'
      : '轩宇汇账号首次登录自动开通免费试用，无需等待审批';
  const ssoSubTip = !ssoPolicy
    ? '；首次登录可开通试用或等待管理员开通'
    : ssoPolicy.approvalRequired
      ? '；首次登录需等待管理员开通'
      : '；首次登录自动开通免费试用';
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'device_limit_exceeded':
          setError({ code: e.code, message: '该账号绑定的设备数量已达上限。如需在本机使用，请联系客服增加设备或解绑旧设备。' });
          break;
        default:
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="relative text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">叮当跨境 ERP</h1>
          <p className="text-gray-500 mt-1">账号由客服分配，登录前请先启动客户端</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 space-y-5">
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 space-y-2">
            <div className="flex items-start gap-2">
              ...
              <div className="font-medium">登录前必读</div>
            </div>
            <ul className="list-disc pl-5 space-y-1 text-amber-800">
              <li>必须先启动叮当跨境桌面客户端，否则无法登录。</li>
              <li>本系统不支持自行注册账号，账号由客服统一分配。</li>
              <li>轩宇汇账号与叮当跨境账号不互通，轩宇汇客户请点击下方按钮跳转登录。</li>
              <li>{ssoTip}。</li>
              <li>如需开通账号或申请测试系统，请联系客服。</li>
            </ul>
          </div>
          ...
          {ssoPhase === 'idle' && (
            <p className="text-center text-xs text-gray-500">
              轩宇汇账号与叮当跨境账号不互通，请使用上方按钮跳转登录{ssoSubTip}
            </p>
          )}
        </form>
        ...
        <div className="mt-5 text-center space-y-3 text-xs text-gray-500">
          ...
          <div>
            如需开通账号、测试系统、找回密码、更换设备或续费，请联系您的专属客服。
          </div>
        </div>
      </div>
    </div>
